...
// One dispatch per invocation, in its own function scope: creating the
// call closure inside a loop body hard-crashes the mJS engine (observed:
// script dies with a blank "crashed" on the first drained burst), while
// the identical burst via a named per-call function is stable.
function dispatchOne() {
  if (ACTIVE >= MAX_CALLS || Q.length === 0) return true;
  ...
  try {
    ...
    // Slot taken by something outside the queue — put it back
    ACTIVE--;
    Q.splice(0, 0, job);
    return false;
  }
...
function pump() {
  ...
  after(10, function () {
    PUMP_SCHED = false;
    // Drain every free slot in this one tick: timers really fire at ~65 ms
    // granularity, so one-call-per-tick staggered a 4-light group by ~200 ms
    // (visible ripple, the last light noticeably late). Bursting from timer
    // context is verified safe — the -103 poisoning needs callback context.
    let guard = 0;
    while (ACTIVE < MAX_CALLS && Q.length > 0 && guard < 8) {
      guard++;
      if (!dispatchOne()) { after(200, pump); return; }
    }
    if (Q.length > 0) pump();
  });
}
