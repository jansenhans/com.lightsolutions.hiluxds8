...
let ST = {};
for (let k in CFG) {
  ST[k] = { dimUp: true, ctUp: true, fading: false, ctFading: false, lastTapEnd: 0, downTs: 0, tapPreceded: false, on: null, b: 50, ct: 3150 };
}
...
function applyToggle(cfg, s, on) {
  ...
  s.on = true;
  callAll(cfg, "CCT.Set", "on=true&brightness=" + JSON.stringify(b) + "&ct=" + JSON.stringify(s.ct) + "&transition_duration=" + JSON.stringify(cfg.fadeOn));
}
