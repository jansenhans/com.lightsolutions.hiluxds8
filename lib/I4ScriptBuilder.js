...
function generate(configsByInput) {
  ...
  const hash = crypto.createHash('sha1').update(cfgJson).digest('hex').slice(0, 12);
...
function toggle(cfg) {
  withStatus(cfg, function (st) {
    ...
    // One shared brightness so the group ramps identically
    let b = st && typeof st.brightness === "number" ? Math.round(st.brightness) : 50;
    if (b < cfg.dimFloor) b = cfg.dimFloor;
    callAll(cfg, "CCT.Set", "on=true&brightness=" + JSON.stringify(b) + "&transition_duration=" + JSON.stringify(cfg.fadeOn));
  });
}
