...
function pump() {
  while (ACTIVE < MAX_CALLS && Q.length > 0) {
...
// Freeze on release by re-setting each light at its current level (both
// brightness and colour, so ct sweeps freeze too). CCT.DimStop is avoided:
// on fw 2.0.0 a stopped fade poisons the light — subsequent upward
// brightness commands are silently ignored until an off/on cycle.
  ...
  for (let i = 0; i < cfg.lights.length; i++) {
    (function (ip) {
      safeCall("http://" + ip + "/rpc/CCT.GetStatus?id=0", function (res) {
        if (s.gen !== g || !res || res.code !== 200) return;
        let st = JSON.parse(res.body);
        if (!st || st.output !== true) return;
        // 0.5 is the firmware's minimum transition (anything lower is
        // rejected with -103) — a half-second glide to the freeze level.
        safeCall("http://" + ip + "/rpc/CCT.Set?id=0&on=true&brightness=" + JSON.stringify(Math.round(st.brightness)) + "&ct=" + JSON.stringify(Math.round(st.ct)) + "&transition_duration=0.5");
      });
    })(cfg.lights[i]);
  }
  pushHomey(cfg, 900);
