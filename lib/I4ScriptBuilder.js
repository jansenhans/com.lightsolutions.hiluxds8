...
let ST = {};
for (let k in CFG) {
  ST[k] = { dimUp: true, ctUp: true, fading: false, ctFading: false, lastTapEnd: 0, downTs: 0, tapPreceded: false, on: null, b: 50, ct: 3150, gen: 0, undoOn: null, singleTs: 0, doubleTs: 0, dblDown: false, dblUsed: false, dblSeq: 0 };
}
...
Shelly.addEventHandler(function (ev) {
  ...
  if (e === "btn_down") {
    ...
    // A press soon after a double-tap: the double's preset is cancelled —
    // if this press becomes a hold, it is the colour-sweep gesture.
    s.dblDown = ts > 0 && s.doubleTs > 0 && (ts - s.doubleTs) < TAP_HOLD_GAP;
    if (s.dblDown) s.dblUsed = true;
    s.downTs = ts;
  } else if (e === "btn_up") {
    if (ts > 0 && s.downTs > 0 && (ts - s.downTs) < 0.35) s.lastTapEnd = ts;
    stopSweep(cfg, s);
  } else if (e === "long_push") {
    // Colour sweep = double-tap then hold (two taps emit only double_push,
    // so no stray toggle can fire — chosen over tap+hold, whose tap the
    // fw 2.0.0 multi-push window turns into a visible toggle+undo dip).
    if (s.dblDown) {
      startCt(cfg, s);
    } else {
      // A hold after a single tap is one compound dim gesture — the tap's
      // stray toggle is undone first.
      undoToggle(cfg, s, ts);
      startDim(cfg, s);
    }
