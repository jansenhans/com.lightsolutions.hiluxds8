...
async function deploy(address, code, hash, log = () => {}) {
  ...
  if (existing) {
    ...
    if (current && typeof current.data === 'string' && current.data.includes(`hash:${hash}`)) {
      // A reboot mid-upload leaves a truncated body behind the intact hash
      // line (and it can still parse and "run" if cut inside a comment), so
      // the hash alone doesn't prove the deploy completed — check length too.
      const total = current.data.length + (current.left || 0);
      if (total === code.length) return { changed: false, id: existing.id };
      log(`i4 ${address}: hash matches but on-device length ${total} != ${code.length} — redeploying`);
    }
  }
